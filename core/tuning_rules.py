"""
SQL 텍스트 기반 튜닝 규칙 엔진
실행 계획 분석 외에 SQL 문법 패턴으로도 문제를 감지합니다.
"""
...
class SqlTextAnalyzer:
    """
    SQL 텍스트를 파싱하여 안티패턴을 감지합니다.
    실행 계획 없이도 동작하므로 1차 검증에 유용합니다.
    """
    ...
    def analyze(self, sql: str) -> list[SqlIssue]:
        ...
        issues += self._check_distinct_overuse(sql_upper)
    ...
    def _check_implicit_conversion(self, sql: str) -> list[SqlIssue]:
        """묵시적 형변환 패턴 감지 (숫자 컬럼 = '문자', 문자 컬럼 = 숫자)"""
        # 간단한 패턴: 컬럼 = '숫자' 형태 (자주 발생하는 케이스)
    ...
    def _check_distinct_overuse(self, sql: str) -> list[SqlIssue]:
    ...
    def _check_scalar_subquery(self, sql: str) -> list[SqlIssue]:
        ...
        # SELECT 절에서 (SELECT ... FROM ...) 패턴 개수
        select_clause = re.split(r'\bFROM\b', sql, maxsplit=1, flags=re.IGNORECASE)[0]
        count = len(re.findall(r'\(\s*SELECT\b', select_clause, re.IGNORECASE))
        if count >= 3:
            return [SqlIssue(
                severity='MEDIUM',
                category='스칼라 서브쿼리',
    ...
    def _check_missing_where(self, sql: str) -> list[SqlIssue]:
        ...
        is_update_delete = re.match(r'\s*(UPDATE|DELETE)\b', sql, re.IGNORECASE)
        has_where = bool(re.search(r'\bWHERE\b', sql, re.IGNORECASE))
        if is_update_delete and not has_where:
            op = is_update_delete.group(1).upper()
            return [SqlIssue(
                severity='HIGH',
                category='전체 테이블 변경',
                ...
                suggestion=(
                    ...
                    f"테스트 전 SELECT로 영향 행 수를 먼저 확인하는 것을 권장합니다."
                ),
            )]
        return []
